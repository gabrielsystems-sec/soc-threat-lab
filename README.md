...
| Resposta Automática | Módulo Firewall-Drop | Bloqueio Automatizado | Bloquear **automaticamente** IPs atacantes em tentativas de força bruta | 
| Notificação de Incidentes com Telegram Bot e Alertas em Tempo Real | Notificar a equipe de segurança **imediatamente** sobre eventos críticos |

---
# 📁 1. Implantação Essencial & Conectividade Segura

## Contexto
Fase inicial **para resolver o desafio de** estabelecer e garantir a comunicação segura entre o servidor central (Wazuh Manager) e os sistemas monitorados, **essencial para a coleta de telemetria crítica e a visibilidade operacional.**
...
- Resolução: Padronização das versões para garantir total compatibilidade, ajustes de permissões no arquivo de configuração ossec.conf e validação do processo de autenticação via authd, **restabelecendo a comunicação e a coleta de dados.**
...
- Análise de Logs de Autenticação (SSH/PAM) para **identificar** acessos negados. 
...
- Tentativas de Brute Force **detectadas** no Host (Wazuh Dashboard): ![HIDS Autenticação](./docs/assets/04-hids-detection-auth.png)
</details> 
---
...
## Contexto
Ataques automatizados acontecem rápido demais para depender de ação humana manual. Esta etapa foca em programar o sistema para reagir sozinho em segundos, **minimizando o tempo de exposição a ameaças.**

## Troubleshooting — Problemas com Sintaxe XML e Integração de APIs

### O Problema
O serviço do Wazuh Manager parou de iniciar após a configuração das integrações com as ferramentas de inteligência externas, **comprometendo a capacidade de detecção e resposta do SIEM.**

### Diagnóstico das Causas Raiz
1. Caracteres invisíveis (Non-breaking spaces) **foram inseridos** no arquivo ossec.conf na hora da edição, o que impedia o sistema de ler o arquivo corretamente.
...
- Uso do comando de teste `wazuh-analysisd -t` para checar se o arquivo estava perfeito antes de reiniciar o serviço, **resultando na rápida restauração da funcionalidade e estabilidade do SIEM.**
...
## Contexto
Testar a eficiência das regras criadas simulando técnicas reais de ataque em um ambiente controlado, **validando a capacidade de detecção e resposta do SIEM.** 
...
Disparo de ataques rápidos de dicionário por SSH usando a ferramenta Hydra, validando se o alarme **seria disparado** no bot do Telegram.
...
📁 7. Hardening de Rede, Decepção & Manutenção Operacional
...
- **Troubleshooting de Acesso:** Depuração de bloqueios de rede causados por políticas restritivas do SELinux que impediam a comunicação entre o balanceador e os serviços internos, **restaurando a conectividade crítica.**
...
- **Diagnóstico de Portas:** Verificação de status de sockets (ss) para validar portas 1514/1515 após ajustes. ![Diagnóstico](./docs/assets/wazuh-port-diagnosis-ss.png)
